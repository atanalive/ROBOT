...
void moving_track()
{
	switch(PATH_NOW.now_path)
  {
		...
		case calibration://矫正初始角
		  ...
		  if(ROBOT_NOW.Vy>=-35)
			{
				mpu_dmp_get_yaw_data(&ROBOT_NOW.initial_yaw);//矫正初始角
...
u8 right_double_online_track(u8 v1,u8 v2)
{
	...
	else if((feedback.infrared_right&0x07)==0x01)//需前移
	{
		ROBOT_NOW.target_Vy=v2;
	}
	else if((feedback.infrared_right&0x07)==0x04)//需后移
	{
		ROBOT_NOW.target_Vy=-v2;
	}
	else if((feedback.infrared_right&0x07)==0x02)//压线
	{
		ROBOT_NOW.target_Vy=0;
		return 1;
	}
	else if((feedback.infrared_right&0x07)==0x07)//全压线
	{
		ROBOT_NOW.target_Vy=0;
		return 2;
	}
...
u8 front_online_track(u8 v1,u8 v2)
{
	...
	else if((feedback.infrared_front&0x70)==0x10)//需右移
	{
		ROBOT_NOW.target_Vx=v2;
	}
	else if((feedback.infrared_front&0x70)==0x40)//需左移
	{
		ROBOT_NOW.target_Vx=-v2;
	}
	else if((feedback.infrared_front&0x70)==0x20)//压线
	{
		ROBOT_NOW.target_Vx=0;
		return 1;
	}
	else if((feedback.infrared_front&0x70)==0x70)//全压线
	{
		ROBOT_NOW.target_Vx=0;
		return 2;
	}
...
//开始路段封装函数 vx为x轴速度 vy为y轴速度 delay1为x轴位移延时 delay2为y轴位移延时
void middle_to_grab_moving(u8 vx,u8 vy,u32 delay1,u32 delay2,enum LOCATION_PATH target)
{
	if(delay_moving(delay1))
	{
		...
		{
			time_control=time.taskcount;//重新赋值，以备下次使用
			mpu_dmp_get_yaw_data(&ROBOT_NOW.initial_yaw);//矫正初始角
...
{
	if(right_count_black_track(1))//第一段,高速
	{
		if(right_count_black_track(2))//第二段,低速
		{
